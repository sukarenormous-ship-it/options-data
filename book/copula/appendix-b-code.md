...
[13] ตัวเลขตัวอย่างในบทที่ 4 และ 8 ตรงกับที่หนังสือเขียนไว้จริง
```
...
def backtest(x, y, window=250, refit_every=20, entry=3.0, exit_at=0.5,
    ...
    delay     : เข้าออกช้ากี่บาร์ — ตั้ง 1 เป็นอย่างน้อยเสมอ (บทที่ 9 หัวข้อ 9.5)
    """
    n = len(x)
...
และนี่คือเหตุผลที่บทที่ 8 หัวข้อ 8.3 เตือนว่า **จุดรีเซ็ต `t₀` เป็นพารามิเตอร์
...
# 1. สัญญาณแรงขึ้นแล้วผลตอบแทนดีขึ้นตามไหม (บทที่ 9, 12)
...
# 2. มี reversion ให้เทรดจริงไหม (บทที่ 9)
...
- **N_eff ที่ tail ออกมา *สูงกว่า* N_eff ปกติ (1.56 เทียบกับ 1.47)** —
  ซึ่งกลับทางกับบทที่ 10 ทั้งบท นี่ไม่ใช่ข้อโต้แย้งบทที่ 10 แต่เป็นการสาธิต
  บทที่ 5 หัวข้อ 5.4: "วันแย่ที่สุด 25%" ของข้อมูล 54 วันคือ 13 วัน
  และ correlation จาก 13 จุดมี error bar กว้างจนทิศทางพลิกได้เอง
  **ถ้าเห็นตัวเลขแบบนี้ในงานจริง คำตอบที่ถูกคือ "ข้อมูลไม่พอ"
  ไม่ใช่ "พอร์ตนี้กระจายความเสี่ยงได้ดีตอน stress"**
