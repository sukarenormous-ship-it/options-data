...
# ตัวเลขที่ใช้เป็นตัวอย่างในบทที่ 8 ต้องตรงกับที่หนังสือเขียนไว้
h_a = float(Clayton(2.0).h(np.array(0.15), np.array(0.60)))
h_b = float(Clayton(2.0).h(np.array(0.15), np.array(0.10)))
check("บทที่ 8 ตัวอย่าง A: h(0.15|0.60) ≈ 0.0147", abs(h_a - 0.0147) < 0.0005, f"got {h_a:.4f}")
check("บทที่ 8 ตัวอย่าง B: h(0.15|0.10) ≈ 0.582", abs(h_b - 0.582) < 0.002, f"got {h_b:.4f}")
